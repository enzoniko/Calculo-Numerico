import math
f_vector = [lambda x1, x2: math.exp(x1) + x2 - 1, lambda x1, x2: x1**2 + x2**2 - 4]

jacobian_matrix = [
    [lambda x1, x2: (f_vector[0](x1 + 0.1, x2) - f_vector[0](x1, x2))/0.1, lambda x1, x2: (f_vector[0](x1, x2+0.1) - f_vector[0](x1, x2))/0.1],
    [lambda x1, x2: (f_vector[1](x1 + 0.1, x2) - f_vector[1](x1, x2))/0.1, lambda x1, x2: (f_vector[1](x1, x2+0.1) - f_vector[1](x1, x2))/0.1]
]
...
def non_linear(x0, erro=1e-15):
    xk = [0]*len(x0)
    k = 0
    while abs(f(x0[0], x0[1])[0]) > erro and abs(f(x0[0], x0[1])[1]) > erro:
        deltas = gauss(jacobian(x0[0], x0[1]), f(x0[0], x0[1]))
